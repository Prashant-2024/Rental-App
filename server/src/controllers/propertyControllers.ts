...
export const getProperties = async (
  ...
  try {
    ...
    if (latitude && longitude) {
      ...
      whereConditions.push(
        Prisma.sql`ST_DWithin(l.coordinates::geometry, ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326), ${degress})`
      );
    }
...
export const createProperty = async (
  ...
  try {
    ...
    const geocodingUrl = `https://nominatim.openstreemap.org/search?${new URLSearchParams(
      {
        street: address,
        city,
        country,
    ...
    const geocodingResponse = await axios.get(geocodingUrl, {
      headers: { "User-Agent": "RealEstateApp (rtlearntest@gmail.com)" },
    });
